...
ENV VIRTUAL_ENV=/opt/scenery-env
ENV PATH=$VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/scenery/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/scenery/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/scenery/ && touch src/scenery/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/scenery/
...
# Expose the app.
ENTRYPOINT ["/opt/scenery-env/bin/scenery"]
CMD []
